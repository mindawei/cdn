...
public class GreedyOptimizerSimple extends GreedyOptimizer{
	...
	private void transfer(int consumerId,Server fromServer, Map<Integer, Server> toServers) {
		...
		while (true) {
			...
			// 是服务器
			int usedDemand = useBandWidthByPreNode(fromDemand, serverNode,Global.allPreNodes[consumerId]);
			...
			if (usedDemand > 0) {
				...
				transferTo(fromServer, toServers.get(serverNode), usedDemand,serverNode,Global.allPreNodes[consumerId]);
