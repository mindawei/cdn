...
public class Checker {
	...
	public static void main(String[] args) {
	     ...
	     for(int consumerId=0;consumerId<Global.consumerNum;++consumerId){
	    	 consumerDemands.put(consumerId, Global.consumerDemands[consumerId]);
	     }
...
//	     }
	     ...
	     for(int i=2;i<resultContent.length;++i){
	    	 ...
	    	 int serverLevel = Integer.parseInt(parts[size-2]) ;
	    	 ...
	    	 if(consumerDemands.get(consumerId)==null){
	    		 System.out.println(consumerDemands);
	    	 ...
	    	 if(serverNodeToLevel.containsKey(serverNode)){
	    		 if(serverNodeToLevel.get(serverNode)!=serverLevel){
	    			 ...
	    			 System.out.println("serverNode "+serverNode+" level0:"+serverNodeToLevel.get(serverNode)+" level1:"+serverLevel);
	    			 System.exit(0);
	    		 }
	    	 }else{
	    		 ...
	    		 nodeCost += Global.nodeDeployCosts[serverNode];
	    		 serverCost += Global.serverDeployCosts[serverLevel];
	     ...
	     for(Map.Entry<Integer,Integer> consumerDemand : consumerDemands.entrySet()){
	    	 int  consumerId = consumerDemand.getKey();
	    
	    	 if(consumerDemand.getValue()>0&&consumerDemand.getValue()<Global.consumerDemands[consumerId]){
