...
public final class OptMiddle {
		...
		while (System.currentTimeMillis() < Global.TIME_OUT) {
			...
			for (serverLevel = Global.serverLevelNum - 1; serverLevel >= 0; --serverLevel) {
				...
				for (int i = 0; i < serverNodesSize; ++i) {
					...
					// 自己到自己
					if (serverLevel != fromLevel) {
						cost = costAfterMove(fromNode, fromNode, serverLevel);
						if (cost != Global.INFINITY) { // 表示成功
							minCost = cost;
							lastToNode = fromNode;
							if (Global.IS_DEBUG) {
	...
	public OptMiddle(int[] nodes, int maxUpdateNum) {
